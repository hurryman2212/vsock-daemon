...
int main(int argc, char *argv[]) {
  ...
  uint64_t host_hash;
  _rdrand64_step((unsigned long long *)&host_hash);
  DEBUG_PRINT("host_hash <- 0x%lx\n", host_hash);

  ev_set.events = EPOLLRDHUP; // EPOLLHUP & EPOLLERR does not work!
  ...
  while (1) {
    ...
    for (int i = 0; i < nr_event; ++i) {
      ...
      if (fd == server_sock) {
        ...
        DEBUG_PRINT("[guest_cid: %u / guest_port: %u] accept: Accepted!\n",
                    client_addr.svm_cid, client_addr.svm_port);

        send(client_sock, &host_hash, sizeof(host_hash), MSG_NOSIGNAL);
        ...
        if (event & EPOLLRDHUP) {
          DEBUG_PRINT(
              "[guest_cid: %u / guest_port: %u] EPOLLRDHUP -> Closing...\n",
              client_addr.svm_cid, client_addr.svm_port);
          if (close(fd)) {
            perror("close");
            exit(EXIT_FAILURE);
